...
const MOCK_ORDERS = [
  ...
  { id: 'CARGO-5555', date: '2026-06-02', user: 'Piotr Wiśniewski', amount: 100, status: 'oczekuje na płatność', payment: 'gotówka', items: 'Rower Cargo + Daszek' }
];

const MOCK_USERS = [
  { id: 'u1', name: 'Jan Kowalski', email: 'jan@example.com', phone: '123 456 789' },
  { id: 'u2', name: 'Anna Nowak', email: 'anna@example.com', phone: '987 654 321' },
  { id: 'u3', name: 'Piotr Wiśniewski', email: 'piotr@example.com', phone: '555 444 333' }
];
...
export default function App() {
  ...
  // Nowe stany rozbudowanego Panelu i Sklepu
  const [products, setProducts] = useState(PRODUCTS);
  const [globalBlockedDates, setGlobalBlockedDates] = useState(['2026-12-24', '2026-12-25', '2026-12-26']); // Domyślne blokady (np. święta)
  const [usersList, setUsersList] = useState(MOCK_USERS);

  // Pomocnicza funkcja generująca pełną listę dat pomiędzy start a end
  ...
  const ProductDetailView = () => {
    ...
    const requestedDates = useMemo(() => {
      ...
      if (new Date(itemEndDate) < new Date(itemStartDate)) return [];
      return getDatesInRange(itemStartDate, itemEndDate);
    }, [itemStartDate, itemEndDate]);

    // Sprawdzamy czy wybrany przedział nie nachodzi na żadną zajętą datę (lub globalne zamknięcie sklepu)
    const isAvailable = useMemo(() => {
      if (requestedDates.length === 0) return null;
      // Odnosimy się do aktualnego stanu products (zmienianego przez admina), a nie stałej PRODUCTS
      const dynamicProduct = products.find(p => p.id === selectedProduct.id) || selectedProduct;
      const productBlocked = dynamicProduct.bookedDates || [];
      const hasConflict = requestedDates.some(d => productBlocked.includes(d) || globalBlockedDates.includes(d));
      return !hasConflict;
    }, [requestedDates, selectedProduct, products, globalBlockedDates]);
    ...
    const handleAddToCart = () => {
      ...
      if (isAvailable === false) { alert("Wybrany termin jest niedostępny (koliduje z innymi rezerwacjami lub przerwą sklepu)."); return; }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-3xl p-8 mb-8 shadow-inner">
              ...
              {itemStartDate && itemEndDate && isAvailable === false && (
                 <div className="mt-6 p-4 bg-red-100 text-red-800 rounded-xl flex items-center text-sm font-semibold border border-red-200">
                   <AlertCircle className="w-5 h-5 mr-2.5" /> Niestety, sprzęt jest niedostępny w całości lub części wybranego terminu.
                 </div>
              )}
              {itemStartDate && itemEndDate && isAvailable === true && (
  ...
  const CheckoutView = () => {
    ...
    const generateContractText = () => {
      const itemsStr = cart.map(item => `  - ${item.name} (od ${item.startDate} do ${item.endDate}, ${item.rentalDays} dni)`).join('\n');
      return `# UMOWA NAJMU SPRZĘTU ROWEROWEGO

Zawarta w dniu ${new Date().toLocaleDateString('pl-PL')} w Radzyminie pomiędzy:

Wynajmującym: 
Michał Mleczko, właściciel platformy cargo.mleczki.pl
(zwany dalej "Wynajmującym")

Najemcą: 
Imię i nazwisko: ${userDetails.name || '[Brak danych - wypełnij formularz]'}
Adres zamieszkania: ${userDetails.address || '[Brak danych - wypełnij formularz]'}
Telefon: ${userDetails.phone || '[Brak danych]'}
E-mail: ${userDetails.email || '[Brak danych]'}
(zwany dalej "Najemcą")

## §1 Przedmiot najmu
Wynajmujący oddaje Najemcy do używania następujący sprzęt:
${itemsStr}

## §2 Czas trwania umowy i opłaty
1. Umowa zawarta jest na czas ściśle określony powyżej.
2. Łączna kwota z tytułu najmu wynosi: ${finalTotal} PLN brutto.
3. Kwota ta płatna jest z góry poprzez system BLIK lub gotówką przy odbiorze.

## §3 Zasady i Odpowiedzialność
1. Najemca oświadcza, że posiada niezbędną wiedzę do eksploatacji powierzonego sprzętu, w tym do przewozu dzieci.
2. Najemca ponosi pełną odpowiedzialność materialną za uszkodzenia, kradzież lub zgubienie sprzętu w okresie najmu, niewynikające z normalnego zużycia eksploatacyjnego.
3. Najemca zobowiązuje się zwrócić sprzęt w stanie niepogorszonym.
4. Zabronione jest podnajmowanie sprzętu osobom trzecim bez wiedzy Wynajmującego.

Podpis Wynajmującego: 
Michał Mleczko (Akceptacja automatyczna)

Podpis Najemcy: 
${userDetails.name || '_________________________'} (Akceptacja elektroniczna przez system)
`;
    };
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <div className="space-y-4 border-t border-gray-100 pt-8">
              <div className="bg-blue-50 border border-blue-200 p-5 rounded-xl mb-6">
                <h4 className="font-bold text-blue-900 mb-2 flex items-center"><FileText className="w-5 h-5 mr-2"/> Wygenerowana Umowa Najmu</h4>
                <p className="text-sm text-blue-800 mb-4">Zgodnie z wymogami prawa, dla każdego zamówienia generujemy umowę najmu z automatycznym uzupełnieniem Twoich danych z formularza. Kliknij przycisk poniżej, aby ją przeczytać.</p>
                <button onClick={() => setIsContractModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors shadow">Otwórz Podgląd Umowy (Markdown)</button>
              </div>
              ...
              <label className="flex items-start space-x-3 cursor-pointer"><input type="checkbox" checked={contractAccepted} onChange={(e) => setContractAccepted(e.target.checked)} className="mt-1 w-5 h-5 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500" /><span className="text-sm text-gray-700">Oświadczam, że zapoznałem/am się z treścią wygenerowanej Umowy Najmu i w pełni akceptuję jej warunki (podpis elektroniczny).*</span></label>
              <label className="flex items-start space-x-3 cursor-pointer"><input type="checkbox" checked={userDetails.acceptTos} onChange={(e) => setUserDetails({...userDetails, acceptTos: e.target.checked})} className="mt-1 w-5 h-5 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500" /><span className="text-sm text-gray-700">Akceptuję Regulamin sklepu i wyrażam zgodę na przetwarzanie danych osobowych (RODO).*</span></label>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="lg:w-1/3 mt-12 lg:mt-0">
          <div className="bg-gray-900 rounded-3xl p-8 text-white sticky top-24 shadow-2xl">
            ...
            <button onClick={() => {
              ...
              if (!userDetails.isAdult || !userDetails.acceptTos || !contractAccepted) {
                alert('Musisz potwierdzić pełnoletność, zaakceptować regulamin oraz Umowę Najmu.'); return;
              }
              handleCheckoutSubmit();
            }} className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-4 px-4 rounded-xl transition-colors flex items-center justify-center text-lg shadow-md">
      ...
      {isContractModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl p-8 max-w-3xl w-full shadow-2xl relative max-h-[90vh] flex flex-col">
            <button onClick={() => setIsContractModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-900"><X className="w-6 h-6" /></button>
            <h2 className="text-2xl font-black mb-4 text-gray-900 flex items-center"><FileText className="w-6 h-6 mr-2 text-emerald-600"/> Podgląd Umowy Najmu</h2>
            <div className="bg-gray-50 p-6 rounded-xl overflow-y-auto whitespace-pre-wrap font-mono text-sm text-gray-800 border border-gray-200 flex-grow shadow-inner">
              {generateContractText()}
            </div>
            <div className="mt-6 flex space-x-4">
              <button onClick={() => setIsContractModalOpen(false)} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-4 rounded-xl transition-colors">Zamknij podgląd</button>
              <button onClick={() => { setContractAccepted(true); setIsContractModalOpen(false); }} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-4 rounded-xl transition-colors">Akceptuję treść umowy</button>
            </div>
          </div>
        </div>
      )}
  ...
  const AdminPanelView = () => {
    const [activeTab, setActiveTab] = useState('orders'); // orders, users, fleet, settings
    const [selectedAdminUser, setSelectedAdminUser] = useState(null);
    const [selectedAdminProduct, setSelectedAdminProduct] = useState(null);
    const [manualDate, setManualDate] = useState('');
    
    // Stany formularza nowej ręcznej rezerwacji z poziomu panelu CRM
    const [newResProduct, setNewResProduct] = useState('');
    const [newResStart, setNewResStart] = useState('');
    const [newResEnd, setNewResEnd] = useState('');
    const [newResPrice, setNewResPrice] = useState('');

    const markOrderConfirmed = (id) => {
      setAdminOrders(adminOrders.map(o => o.id === id ? { ...o, status: 'potwierdzone ręcznie' } : o));
    };

    const handleAddManualReservation = (e) => {
      e.preventDefault();
      if(!newResProduct || !newResStart || !newResEnd || !newResPrice) return;
      
      const datesToBlock = getDatesInRange(newResStart, newResEnd);
      // Aktualizacja dat w głównym obiekcie (blokada terminów)
      setProducts(products.map(p => {
        if (p.id === newResProduct) {
          return { ...p, bookedDates: [...(p.bookedDates || []), ...datesToBlock] };
        }
        return p;
      }));

      // Dodanie symulowanego zamówienia do widoku Admina
      setAdminOrders([{
        id: `MANUAL-${Math.floor(Math.random()*1000)}`,
        date: new Date().toISOString().split('T')[0],
        user: selectedAdminUser.name,
        amount: parseFloat(newResPrice),
        status: 'zaakceptowane',
        payment: 'manualna (gotówka)',
        items: products.find(p=>p.id===newResProduct)?.name
      }, ...adminOrders]);

      alert('Ręczna rezerwacja dodana pomyślnie. Terminy w kalendarzu sklepu zostały zablokowane!');
      setNewResProduct(''); setNewResStart(''); setNewResEnd(''); setNewResPrice('');
    };

    const toggleProductBlockDate = (productId, dateToToggle) => {
      setProducts(products.map(p => {
        if (p.id === productId) {
          const currentDates = p.bookedDates || [];
          const isBlocked = currentDates.includes(dateToToggle);
          return {
            ...p,
            bookedDates: isBlocked ? currentDates.filter(d => d !== dateToToggle) : [...currentDates, dateToToggle]
          };
        }
        return p;
      }));
    ...
    const toggleGlobalBlockDate = (dateToToggle) => {
      const isBlocked = globalBlockedDates.includes(dateToToggle);
      if (isBlocked) {
        setGlobalBlockedDates(globalBlockedDates.filter(d => d !== dateToToggle));
      } else {
        setGlobalBlockedDates([...globalBlockedDates, dateToToggle]);
      }
    };
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex space-x-2 mb-8 border-b border-gray-200 pb-2 overflow-x-auto">
          <button onClick={() => { setActiveTab('orders'); setSelectedAdminUser(null); setSelectedAdminProduct(null); }} className={`px-4 py-2 font-bold rounded-t-lg ${activeTab==='orders' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>Zamówienia & Wpłaty</button>
          <button onClick={() => { setActiveTab('users'); setSelectedAdminProduct(null); }} className={`px-4 py-2 font-bold rounded-t-lg ${activeTab==='users' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>Klienci (CRM)</button>
          <button onClick={() => { setActiveTab('fleet'); setSelectedAdminUser(null); }} className={`px-4 py-2 font-bold rounded-t-lg ${activeTab==='fleet' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>Zarządzanie Flotą</button>
          <button onClick={() => { setActiveTab('settings'); setSelectedAdminUser(null); setSelectedAdminProduct(null); }} className={`px-4 py-2 font-bold rounded-t-lg ${activeTab==='settings' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>Globalne Zamknięcia Sklepu</button>
        </div>
        ...
        {activeTab === 'orders' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {adminOrders.map(order => (
                  <div key={order.id} className="border border-gray-100 bg-gray-50 p-4 rounded-xl">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${order.status === 'opłacone' || order.status.includes('potwierdzone') ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.status.toUpperCase()}</span>
                    </div>
                    {/* Klikalny klient przenoszący do zakładki CRM */}
                    <p className="text-sm text-gray-600 mb-1 cursor-pointer hover:text-emerald-600 font-medium transition-colors inline-flex items-center" 
                       onClick={() => { setActiveTab('users'); setSelectedAdminUser(usersList.find(u => u.name === order.user) || { name: order.user, email: 'Brak w bazie', phone: 'Brak' }); }}>
                      <User className="w-4 h-4 mr-1"/> {order.user} <ArrowRight className="w-3 h-3 ml-1 opacity-50"/>
                    </p>
                    <p className="text-sm text-gray-600 mb-3 flex items-center"><CargoBikeIcon className="w-4 h-4 mr-1"/> {order.items}</p>
                    
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-2 pt-3 border-t border-gray-200 gap-3">
                      <span className="font-bold">{order.amount} zł ({order.payment})</span>
                      <div className="flex space-x-2">
                        {order.status === 'oczekuje na płatność' && (
                          <button onClick={() => markOrderConfirmed(order.id)} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 transition shadow">Potwierdź (po tel.)</button>
                        )}
                        {order.status !== 'opłacone' && (
                          <button onClick={() => markOrderPaid(order.id)} className="text-xs bg-emerald-600 text-white px-3 py-1.5 rounded-lg hover:bg-emerald-700 transition shadow">Oznacz opłacone</button>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-7">
              <h3 className="font-bold text-xl mb-6 border-b pb-2 flex items-center justify-between"><span>Parsowanie z Emaila (BLIK)</span><span className="text-xs font-normal bg-gray-200 px-2 py-1 rounded-full text-gray-600">Ostatnia sync: 1 min temu</span></h3>
              <div className="space-y-4">
                {MOCK_TRANSFERS.map(t => (
                  <div key={t.id} className="border-l-4 border-emerald-500 bg-gray-50 p-4 rounded-r-xl">
                    <div className="flex justify-between"><span className="text-xs text-gray-500">{t.date}</span><span className={`text-xs font-bold ${t.status === 'matched' ? 'text-emerald-600' : 'text-red-500'}`}>{t.status === 'matched' ? 'DOPASOWANY' : 'BRAK DOPASOWANIA'}</span></div>
                    <p className="font-bold text-gray-900 mt-1">{t.amount} PLN</p>
                    <p className="text-sm text-gray-600">Od: {t.sender}</p>
                    <p className="text-sm font-mono text-blue-600 bg-blue-50 inline-block px-2 py-0.5 mt-1 rounded">Tytuł: {t.title}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* ZAKŁADKA: Klienci / Profil i Ręczne Rezerwacje */}
        {activeTab === 'users' && !selectedAdminUser && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-7">
            <h3 className="font-bold text-xl mb-6 border-b pb-2">Baza Klientów</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {usersList.map(u => (
                <div key={u.id} onClick={() => setSelectedAdminUser(u)} className="border border-gray-100 p-5 rounded-xl cursor-pointer hover:shadow-lg transition-shadow hover:border-emerald-400 bg-white group">
                  <div className="flex items-center mb-3">
                    <div className="w-12 h-12 bg-emerald-50 group-hover:bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 mr-4 transition-colors"><User className="w-6 h-6"/></div>
                    <div><h4 className="font-bold text-gray-900 text-lg">{u.name}</h4><p className="text-xs text-gray-500">{u.email}</p></div>
                  </div>
                  <p className="text-sm text-gray-600 flex items-center"><Phone className="w-4 h-4 mr-2 text-gray-400"/>{u.phone}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'users' && selectedAdminUser && (
          <div>
            <button onClick={() => setSelectedAdminUser(null)} className="mb-6 text-emerald-600 hover:text-emerald-700 flex items-center font-bold text-sm bg-emerald-50 px-4 py-2 rounded-lg transition-colors"><ArrowLeft className="w-4 h-4 mr-2"/> Wróć do listy klientów</button>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                  <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 mb-6"><User className="w-10 h-10"/></div>
                  <h3 className="text-2xl font-black mb-2">{selectedAdminUser.name}</h3>
                  <p className="text-gray-500 mb-6 text-sm">{selectedAdminUser.email}</p>
                  <div className="space-y-3 border-t border-gray-100 pt-6">
                    <p className="text-sm text-gray-700 flex items-center"><Phone className="w-4 h-4 mr-2 text-emerald-500"/> {selectedAdminUser.phone}</p>
                    <p className="text-sm text-gray-700 flex items-start"><MapPin className="w-4 h-4 mr-2 mt-0.5 text-emerald-500"/> {selectedAdminUser.address || 'Brak wpisanego adresu'}</p>
                  </div>
                </div>
              </div>
              <div className="lg:col-span-2">
                <div className="bg-emerald-50 rounded-2xl shadow-sm border border-emerald-200 p-8 mb-8">
                  <h3 className="font-bold text-xl mb-2 text-emerald-900 flex items-center"><Edit3 className="w-5 h-5 mr-2"/> Utwórz ręczną rezerwację</h3>
                  <p className="text-sm text-emerald-700 mb-6">Użyj tego formularza, jeśli klient zadzwonił, przedłuża wynajem lub chcesz zaoferować mu specjalną zniżkę/nadpisać cenę. Terminy zostaną zablokowane dla innych.</p>
                  
                  <form onSubmit={handleAddManualReservation} className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        <label className="block text-sm font-bold text-emerald-900 mb-1.5">Wybierz Sprzęt</label>
                        <select required value={newResProduct} onChange={(e)=>setNewResProduct(e.target.value)} className="w-full p-3 border border-emerald-300 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-white shadow-sm">
                          <option value="">-- Wybierz model --</option>
                          {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-emerald-900 mb-1.5">Cena całościowa (ugodowa PLN)</label>
                        <input type="number" required value={newResPrice} onChange={(e)=>setNewResPrice(e.target.value)} className="w-full p-3 border border-emerald-300 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-white shadow-sm" placeholder="np. 150" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-emerald-900 mb-1.5">Od dnia</label>
                        <input type="date" required value={newResStart} onChange={(e)=>setNewResStart(e.target.value)} className="w-full p-3 border border-emerald-300 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-white shadow-sm" />
                      </div>
                      <div>
                        <label className="block text-sm font-bold text-emerald-900 mb-1.5">Do dnia</label>
                        <input type="date" required value={newResEnd} onChange={(e)=>setNewResEnd(e.target.value)} className="w-full p-3 border border-emerald-300 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-white shadow-sm" />
                      </div>
                    </div>
                    <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 px-4 rounded-xl transition-colors mt-4 shadow-md text-lg">Utwórz rezerwację i zablokuj kalendarz</button>
                  </form>
                </div>
              </div>
            </div>
        ...
        {/* ZAKŁADKA: Zarządzanie Flotą */}
        {activeTab === 'fleet' && !selectedAdminProduct && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <h3 className="font-bold text-xl mb-2">Zarządzanie Dostępnością Przedmiotów</h3>
            <p className="text-gray-600 mb-6 text-sm">Kliknij wybrany sprzęt, aby oznaczyć go jako niedostępny ("wirtualny wynajem" lub serwis). Te daty znikną natychmiast z kalendarza publicznego.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map(p => (
                <div key={p.id} onClick={() => setSelectedAdminProduct(p)} className="border border-gray-200 rounded-2xl overflow-hidden cursor-pointer hover:shadow-xl transition-shadow hover:border-emerald-400 group">
                  <div className="h-36 overflow-hidden"><img src={p.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="" /></div>
                  <div className="p-5">
                    <h4 className="font-bold text-gray-900 mb-2 line-clamp-1">{p.name}</h4>
                    <span className="inline-flex items-center bg-gray-100 text-gray-700 text-xs font-bold px-2.5 py-1 rounded-lg"><CalendarClock className="w-3.5 h-3.5 mr-1.5"/> Zablokowanych dni: {(p.bookedDates || []).length}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'fleet' && selectedAdminProduct && (
          <div>
            <button onClick={() => setSelectedAdminProduct(null)} className="mb-6 text-emerald-600 hover:text-emerald-700 flex items-center font-bold text-sm bg-emerald-50 px-4 py-2 rounded-lg transition-colors"><ArrowLeft className="w-4 h-4 mr-2"/> Wróć do spisu floty</button>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
              <div className="flex items-center mb-8 border-b pb-6">
                <div className="w-20 h-20 rounded-2xl overflow-hidden mr-5 shadow-sm border border-gray-100"><img src={selectedAdminProduct.image} className="w-full h-full object-cover" alt="" /></div>
                <div>
                  <h3 className="text-2xl font-black mb-1">{selectedAdminProduct.name}</h3>
                  <p className="text-gray-500">Ustalanie niedostępności wybranego modelu (serwis / naprawa / użytek własny)</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                  <h4 className="font-bold text-gray-900 mb-4 flex items-center"><CalendarDays className="w-5 h-5 mr-2 text-gray-500"/> Zablokuj pojedynczy dzień</h4>
                  <div className="flex space-x-3">
                    <input type="date" value={manualDate} onChange={(e)=>setManualDate(e.target.value)} className="flex-grow p-3.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500" />
                    <button onClick={() => { if(manualDate) { toggleProductBlockDate(selectedAdminProduct.id, manualDate); setManualDate(''); } }} className="bg-gray-900 hover:bg-gray-800 text-white px-6 rounded-xl font-bold shadow">Zablokuj dzień</button>
                  </div>
                  <p className="text-xs text-gray-500 mt-3 leading-relaxed">Dodane tu daty uniemożliwią zaznaczenie tego przedmiotu w sklepie. Pamiętaj, aby blokować także dni w trakcie transportu.</p>
                </div>
                <div>
                  <h4 className="font-bold text-gray-900 mb-4 bg-gray-100 px-4 py-2 rounded-lg inline-block">Aktywne Blokady: ({ (products.find(p=>p.id===selectedAdminProduct.id)?.bookedDates || []).length })</h4>
                  <div className="space-y-2 max-h-72 overflow-y-auto pr-2">
                    { (products.find(p=>p.id===selectedAdminProduct.id)?.bookedDates || []).sort().map(d => (
                      <div key={d} className="flex justify-between items-center bg-white border border-gray-200 p-4 rounded-xl shadow-sm hover:border-emerald-200 transition-colors">
                        <span className="font-mono text-sm font-bold text-gray-700">{d}</span>
                        <button onClick={() => toggleProductBlockDate(selectedAdminProduct.id, d)} className="text-xs font-bold text-red-500 hover:text-white hover:bg-red-500 border border-red-200 px-3 py-1.5 rounded transition-colors">Odblokuj / Zwolnij termin</button>
                      </div>
                    ))}
                    {!(products.find(p=>p.id===selectedAdminProduct.id)?.bookedDates || []).length && <p className="text-sm text-gray-400 font-medium p-4 border border-dashed rounded-xl border-gray-300 text-center">Brak zablokowanych terminów. Sprzęt wolny każdego dnia.</p>}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* ZAKŁADKA: Globalne Zamknięcia Sklepu */}
        {activeTab === 'settings' && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <h3 className="font-bold text-2xl mb-2 text-red-600 flex items-center"><Target className="w-6 h-6 mr-2"/> Globalne Zamykanie Sklepu</h3>
            <p className="text-gray-600 mb-8 max-w-3xl">Zarządzaj dniami, w których wypożyczalnia jest <b>całkowicie nieczynna</b> (np. urlop, wyjazd, święta). W te dni absolutnie żaden sprzęt z oferty nie będzie dostępny do rezerwacji online i nie ukaże się w kalendarzach.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="bg-red-50 p-6 rounded-2xl border border-red-100">
                <h4 className="font-bold text-red-900 mb-4 flex items-center"><CalendarDays className="w-5 h-5 mr-2"/> Wyznacz dzień urlopowy</h4>
                <div className="flex space-x-3">
                  <input type="date" value={manualDate} onChange={(e)=>setManualDate(e.target.value)} className="flex-grow p-3.5 border border-red-200 rounded-xl focus:ring-2 focus:ring-red-500 bg-white" />
                  <button onClick={() => { if(manualDate) { toggleGlobalBlockDate(manualDate); setManualDate(''); } }} className="bg-red-600 hover:bg-red-700 text-white px-8 rounded-xl font-bold shadow-md">Zamknij Sklep</button>
                </div>
              </div>
              
              <div>
                <h4 className="font-bold text-gray-900 mb-4">Zamknięte dni w kalendarzu ({globalBlockedDates.length})</h4>
                <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
                  { [...globalBlockedDates].sort().map(d => (
                    <div key={d} className="flex justify-between items-center bg-white border border-red-200 p-4 rounded-xl shadow-sm">
                      <span className="font-mono text-sm font-bold text-red-700">{d}</span>
                      <button onClick={() => toggleGlobalBlockDate(d)} className="text-sm font-bold text-gray-500 hover:text-gray-800 underline">Otwórz sklep w ten dzień</button>
                    </div>
                  ))}
                  {globalBlockedDates.length === 0 && <p className="text-sm text-gray-500">Sklep działa bez zakłóceń każdego dnia.</p>}
                </div>
              </div>
            </div>
          </div>
